...
export async function lintCommand(schemaPath?: string) {
  ...
  try {
    ...
    try {
      ...
      if (err.code === "ENOENT") {
        spinner.fail(chalk.red(`Schema file not found at ${targetPath}. Please provide a valid schema file.`));
        process.exit(1);
      }
      throw err;
    }
    ...
    while ((match = idRegex.exec(content)) !== null) {
      spinner.warn(chalk.yellow(`Warning: Found manual id definition "${match[0]}". ZerithDB automatically handles _id using UUID v7. Defining it manually is an anti-pattern.`));
      warnings++;
    }
    ...
    while ((collectionMatch = collectionsRegex.exec(content)) !== null) {
      ...
      if (fields.length > 5 && !fieldsBlock.includes("&") && !fieldsBlock.includes("*")) {
         spinner.warn(chalk.yellow(`Warning: Collection "${collectionName}" has many fields but no apparent indexes (like & or *). Consider indexing frequently queried fields for better performance.`));
         warnings++;
      }
    }
